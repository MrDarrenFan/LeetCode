...
public class Solution1024 {
    ...
    // 动态规划
    // O(T×N)，其中 T 是区间的长度，N 是子区间的数量。
    // O(T)，其中 T 是区间的长度。
    public int videoStitching(int[][] clips, int T) {
        int numOfClips = clips.length;  // 片段数
        int[] dp = new int[T + 1];  // dp[i] 表示覆盖区间[0,i)所需要的最少片段数
        Arrays.fill(dp, 1, T + 1, Integer.MAX_VALUE - 1);   // dp[0] 为 0，其余初始值可以设为最大整数 - 1（避免加1时溢出）
        // 根据dp每个位置，再遍历每个片段，若位置i包含在某个片段[a, b]内，则取dp[i] = min(dp[i], dp[a] + 1)
        for (int i = 1; i < T + 1; i++) {
