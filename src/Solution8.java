...
public class Solution8 {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(solution8.myAtoi("-+12"));
    ...
    // 方法一：
    // 正常处理，但由于情况较多，容易写出多个if-else，造成代码臃肿
    public int myAtoi(String s) {
        ...
        s = s.trim();
        // 字符串仅包含空白字符时
        // 或 第一个非空格字符不是一个有效整数字符 时
        if (s.length() == 0
                || ((s.charAt(0) < '0' || s.charAt(0) > '9') && (s.charAt(0) != '-' && s.charAt(0) != '+'))) {
            return 0;
        }
        // 记录字符串正负，第二个负号无效
        ...
        for (char c : s.toCharArray()) {
            ...
            if (!isPositive && (-result < Integer.MIN_VALUE / 10 || -result == Integer.MIN_VALUE / 10 && c > '8')) {
